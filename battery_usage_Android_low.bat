...
REM 배터리 용량 확인 및 출력
adb shell dumpsys batterystats | findstr /C:"Estimated power use (mAh):" /C:"Capacity:" > battery_capacity.txt
for /f "tokens=2 delims=:" %%a in ('findstr /C:"Capacity:" battery_capacity.txt') do (
    set "battery_capacity=%%a"
)
set "battery_capacity=%battery_capacity:~1%"
...
REM 배터리 사용량 분석
set "battery_usage=0"
for /f "tokens=9 delims=: " %%a in ('findstr /c:"Uid %uid%:" temp_battery_stats.txt ^| findstr "Including smearing"') do (
